...
def SaveClose():
    try:
        ...
        if selected_item_info:
            ...
            cursor.execute('''INSERT INTO Quotation (customerName, Date, CustomerAddress, customerTaxID, CustomerTel, QuotationNumber, AmountBeforVat, Vat, TotalAmount, Description, ProductCode)
...
class QuotationNumberGenerator:
    ...
    def load_quotation_number(self):
        try:
            with open("quotation_counter.json", "r") as file:
                data = json.load(file)
                return data["counter"]
    ...
    def save_quotation_number(self):
        ...
        with open("quotation_counter.json", "w") as file:
            json.dump(data, file)
    ...
    def generate_quotation_number(self):
        ...
        quotation_number = f"QT{formatted_date}-{self.counter:03d}"
...
def write_to_excel_Quotation(name_text, Date, Address, DocumentNO_text, TaxID, tel_text, TotalAmount_text, VATAmount_text, GrandTotalAmount_text):
    ...
    xlsx_template_path = "Quotation_Form.xlsx"
    # กำหนดที่อยู่ของไฟล์ Excel ที่จะบันทึกข้อมูล
    timestamp = datetime.now().strftime("%d_%m_%Y_%H_%M_%S_")

        
    output_path = fr"\\N2N-BANK\Users\Administrator\Downloads\Excel\Quotation\Quotation_{timestamp}.xlsx"
    path = r"\\N2N-BANK\Users\Administrator\Downloads\Excel\Quotation"
    ...
    sheet.cell(row=14, column=9, value=Date)
    sheet.cell(row=14, column=3, value=Address)
    sheet.cell(row=16, column=4, value=TaxID)
    
    sheet.cell(row=36, column=11, value=TotalAmount_text)
    sheet.cell(row=40, column=11, value=VATAmount_text)
    sheet.cell(row=42, column=11, value=GrandTotalAmount_text)
    ...
    sheet.cell(row=43, column=5, value=grand_total_text_thai_with_comma  + '  บาท')
    sheet.cell(row=54, column=2, value="วันที่ : {}".format(Date))
    sheet.cell(row=54, column=7, value="วันที่ : {}".format(Date))
    sheet.cell(row=54, column=9, value="วันที่ : {}".format(Date))
...
image_Quotation = PhotoImage(
    file=relative_to_assets("Quotation.png"))
