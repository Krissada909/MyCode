...
def SaveClose():
    try:
        ...
        if selected_item_info:
            ...
            Description = selected_item_info[1] 
            ...
            cursor.execute('''INSERT INTO Receipt (CustomerName, Date, CustomerAddress, CustomerTaxID, CustomerTel, ReceiptNumber, TotalAmount, Description)
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?)''', (name_text, Date, Address_text, tax_id, tel_text, DocumentNO_text, GrandTotalAmount_text, Description))
            ...
            write_to_excel_Quotation(name_text, Date, Address_text, DocumentNO_text, tax_id, tel_text, GrandTotalAmount_text, Remarks_Text)
            

            import SingnatureReceipt
...
class Products:
    def search_Product(search_keyword=None):
        try:
            ...
            if search_keyword:
                ...
                query = f"SELECT InvoiceNumber, Description, TotalAmount FROM Invoice WHERE InvoiceNumber LIKE ? OR InvoiceNumber LIKE ?"
                cursor.execute(query, ('%' + search_keyword + '%', '%' + search_keyword + '%'))
            else:
                query = f"SELECT InvoiceNumber, Description, TotalAmount FROM Invoice"
    ...
    def addnewProducts():
        ...
        while True:
            ...
            if popup_event in (sg.WIN_CLOSED, "ยกเลิก"):
                ...
                for tag in ["product_item", "Description_Text", "InvoiceNumber_Text", "TotalAmount_Text", "delete_button", "delete_Text"]:
                    canvas.delete(tag)
                
                selected_items_listbox = popup_values.get('popup_items_listbox', [])
                if selected_items_listbox:
                    for selected_item_name in selected_items_listbox:
                        if selected_item_name:
                            selected_item_info = next((item for item in items if item[0] == selected_item_name), None)

                            if selected_item_info:
                                print(len(selected_item_info))
                                print(selected_item_info)
                                # นำข้อมูลที่เลือกมาเก็บไว้
                                new_row_data = [
                                    f'{len(table_data) + 1}',  # Item (Gen ตัวเลขขึ้นมาเอง)
                                    selected_item_info[0],  # InvoiceNumber
                                    selected_item_info[1],  # Description
                                    selected_item_info[2],  # Total Amount
                                ]
                                ...
                                if not new_row_data:
                                    ...
                                    Description_Text = canvas.create_text(
                                        ...
                                        y_offset,
                                        anchor="nw",
                                        text=new_row_data[2],  # Description
                                        fill="black",
                                        font=("Inter", 14 * -1)
                                    )
                                    canvas.addtag_withtag(f"Description_Text_{len(table_data)}", Description_Text)

                                    InvoiceNumber_Text = canvas.create_text(
                                        185.0,
                                        y_offset,
                                        anchor="nw",
                                        text=new_row_data[1],  # InvoiceNumber
                                        fill="black",
                                        font=("Inter", 14 * -1)
                                    )
                                    canvas.addtag_withtag(f"InvoiceNumber_Text_{len(table_data)}", InvoiceNumber_Text)

                                    TotalAmount_Text = canvas.create_text(
                                        550.0,
                                        y_offset,
                                        anchor="nw",
                                        text=new_row_data[3],  # Total Amount
                                        fill="black",
                                        font=("Inter", 14 * -1)
                                    )
    ...
    # หาตำแหน่งของแถวที่ต้องการลบในตารางข้อมูล table_data
        ...
        if row_id is not None:
                ...
                for item_tag in ["product_item", "Description_Text", "InvoiceNumber_Text", "TotalAmount_Text", "delete_button", "delete_Text"]:
                    canvas.delete(canvas.find_withtag(f"{item_tag}_{row_id + 1}"))  # ใช้ ID ของแถวเป็น tag
                ...
                for i in range(row_id, len(table_data)):
                    ...
                    for item_tag in ["product_item", "Description_Text", "InvoiceNumber_Text", "TotalAmount_Text", "delete_button", "delete_Text"]:
                        canvas.addtag_withtag(f"{item_tag}_{i + 1}", canvas.find_withtag(f"{item_tag}_{i + 2}"))  # เปลี่ยนชื่อ tag ของวัตถุบน Canvas ให้ตรงกับลำดับใหม่
    ...
    def update_gui_with_new_data():
        grand_total_amount = sum(float(row[3].replace(',', '')) for row in table_data)
        GrandTotalAmount.delete(0, "end")
    ...
    def update_canvas_with_new_data():
        ...
        for index, row_data in enumerate(table_data):
            ...
            InvoiceNumber_Text = canvas.create_text(
                ...
                y_offset,
                anchor="nw",
                text=row_data[1],  # InvoiceNumber
                fill="black",
                font=("Inter", 14 * -1)
            )
            canvas.addtag_withtag(f"InvoiceNumber_Text_{index + 1}", InvoiceNumber_Text)

            TotalAmount_Text = canvas.create_text(
                550.0,
                y_offset,
                anchor="nw",
                text=row_data[3],  # Total Amount
                fill="black",
                font=("Inter", 14 * -1)
            )
...
class QuotationNumberGenerator:
    ...
    def load_quotation_number(self):
        try:
            with open("Receipt_counter.json", "r") as file:
                data = json.load(file)
                return data["counter"]
    ...
    def save_quotation_number(self):
        ...
        with open("Receipt_counter.json", "w") as file:
            json.dump(data, file)
    ...
    def generate_quotation_number(self):
        ...
        quotation_number = f"REC{formatted_date}-{self.counter:03d}"
...
def write_to_excel_Quotation(name_text, Date, Address, DocumentNO_text, TaxID, tel_text, GrandTotalAmount_text, Remarks_Text):
    # กำหนดที่อยู่ของไฟล์เทมเพลต Excel
    xlsx_template_path = "Receipt_Form.xlsx"
    # กำหนดที่อยู่ของไฟล์ Excel ที่จะบันทึกข้อมูล
    timestamp = datetime.now().strftime("%d_%m_%Y_%H_%M_%S_")

        
    output_path = fr"\\N2N-BANK\Users\Administrator\Downloads\Excel\Receipt\Receipt_{timestamp}.xlsx"
    path = r"\\N2N-BANK\Users\Administrator\Downloads\Excel\Receipt"
    ...
    sheet.cell(row=12, column=11, value=DocumentNO_text)
    sheet.cell(row=12, column=3, value=name_text)
    sheet.cell(row=14, column=11, value=Date)
    sheet.cell(row=14, column=3, value=Address) 
    sheet.cell(row=16, column=4, value=TaxID)

    sheet.cell(row=36, column=13, value=GrandTotalAmount_text)
    sheet.cell(row=17, column=11, value=Remarks_Text)
    ...
    sheet.cell(row=36, column=5, value=grand_total_text_thai_with_comma  + '  บาท')
    ...
    for i, item_data in enumerate(table_data, start=1):
        ...
        sheet.cell(row=current_row, column=item_col_num, value=item_data[0])  # Item
             ...
             # กำหนด Item 
        InvoiceNumber = 3  # Column C
        sheet.cell(row=current_row, column=InvoiceNumber, value=item_data[1])  # Item

             # กำหนด Item 
        Description = 5  # Column C
        sheet.cell(row=current_row, column=Description, value=item_data[2])  # Item

             # กำหนด Item 
        Total_Amount = 13  # Column C
        sheet.cell(row=current_row, column=Total_Amount, value=item_data[3])  # Item
...
root.geometry("1270x680") # ตั้งค่าให้หน้าต่างเต็มหน้าจอ
...
image_Quotation = PhotoImage(
    file=relative_to_assets("Receipt_Text.png"))
