...
def convert_excel_to_pdf(excel_path, output_pdf_path):
        ...
        excel_app.Visible = False  # ทำให้ Excel มองเห็นอยู่ในระดับนอกสุด
        ...
        workbook.Close(False)  # ปิด Workbook โดยไม่บันทึกการเปลี่ยนแปลง
        excel_app.Quit()  # ออกจาก Excel


def convert_excel_to_pdfmmanuscript(excel_path, output_pdf_path):
    
        # เปลี่ยนที่ตั้งของไฟล์ PDF ให้อยู่ในโฟเดอร์เดียวกับ Excel
        output_pdf_path = os.path.join(os.path.dirname(excel_path), f"pdfmmanuscript.pdf")
...
def SaveClose():
    try:
        ...
        Date = date.get()
        TermsOfPayment_Text = TermsOfPayment.get()
        Reference_Text = Reference.get()
        Remark_Text = Remark_INvoice.get("1.0", "end-1c")
        ...
        if selected_item_info:
            ...
            ProductCode = selected_item_info[3]
            ...
            cursor.execute('''INSERT INTO Invoice (CustomerName, Date, CustomerAddress, CustomerTaxID, CustomerTel, InvoiceNumber, AmountBeforVat, Vat, TotalAmount, Description, ProductCode)
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', (name_text, Date, Address_text, tax_id, tel_text, DocumentNO_text, TotalAmount_text, VATAmount_text, GrandTotalAmount_text, Description, ProductCode))
            ...
            write_to_excel_Quotation(name_text, Date, Address_text, DocumentNO_text, tax_id, tel_text, TotalAmount_text, VATAmount_text, GrandTotalAmount_text, TermsOfPayment_Text,Reference_Text,Remark_Text)
            write_to_excel_manuscript(name_text, Date, Address_text, DocumentNO_text, tax_id, tel_text, TotalAmount_text, VATAmount_text, GrandTotalAmount_text, TermsOfPayment_Text,Reference_Text,Remark_Text)

            import SignatureInvoice
            import SignatureInvoiceManuscipt
...
class Products:
    ...
    def addnewProducts():
        global selected_item_info, table_data, canvas, TotalAmount, VATAmount, GrandTotalAmount, window

        items = Products.search_Product()
        ...
        while True:
            ...
            if popup_event in (sg.WIN_CLOSED, "ยกเลิก"):
                ...
                for tag in ["product_item", "Product_text", "Description_Text", "Quantity_Text", "Amount_Text", "TotalAmount_Text", "delete_button", "delete_Text"]:
                    canvas.delete(tag)
                    ...
                    if selected_item_name:
                        ...
                        if selected_item_info:
                            ...
                            if not new_row_data:
                                ...
                                # เพิ่มฟิลด์อินพุตสำหรับแก้ไขจำนวนสินค้า
                                Quantity_Text = tk.Entry(canvas, width=5)
                                Quantity_Text.insert(0, '1')
                                canvas.create_window(770, Qua, window=Quantity_Text)

                                # ผูกเหตุการณ์เมื่อเปลี่ยนแปลงค่าในฟิลด์อินพุต
                                def update_quantity(event, row_data=new_row_data):
                                    try:
                                        quantity = int(Quantity_Text.get())
                                        amount = float(row_data[4])
                                        total_amount_text = canvas.find_withtag(f"TotalAmount_Text_{row_data[0]}")
                                        canvas.itemconfig(total_amount_text, text=f'{quantity * amount:.2f}')
                                        
                                        # อัปเดตรวมยอดเงินใหม่
                                        row_data[3] = str(quantity)
                                        table_data[int(row_data[0]) - 1] = row_data
                                        
                                        total_amount = sum(float(row[4]) * int(row[3]) for row in table_data)
                                        ...
                                        sg.popup_error("กรุณาใส่จำนวนเป็นตัวเลข")

                                Quantity_Text.bind("<KeyRelease>", update_quantity)
    ...
    def delete_Product(selected_row_data):
        ...
        if row_id is not None:
            ...
            for item_tag in ["product_item", "Product_text", "Description_Text", "Quantity_Text",  "Amount_Text", "TotalAmount_Text", "delete_button", "delete_Text"]:
...
# ฟังก์ชันหรือเมธอดสำหรับแปลงจำนวนเงินเป็นตัวอักษร
def convert_to_text(GrandTotalAmount):
    # แปลงจำนวนเงินให้อยู่ในรูปแบบข้อความ (ตัวอักษร) ภาษาไทย
    return num2words(GrandTotalAmount, lang='th')
    ...
    def AddCustomer():
        ...
        while True:
            ...
            if popup_event == sg.WIN_CLOSED or popup_event == "cancel_button":
                ...
                customers = Customers.search_customers(keyword)  # ค้นหาลูกค้าตามคำค้นหา
...
class QuotationNumberGenerator:
    ...
    def load_quotation_number(self):
        try:
            with open("Invoice_counter.json", "r") as file:
                data = json.load(file)
                return data["counter"]
    ...
    def save_quotation_number(self):
        ...
        with open("Invoice_counter.json", "w") as file:
            json.dump(data, file)
    ...
    def generate_quotation_number(self):
        ...
        quotation_number = f"INV{formatted_date}-{self.counter:03d}"
...
def write_to_excel_Quotation(name_text, Date, Address, DocumentNO_text, TaxID, tel_text, TotalAmount_text, VATAmount_text, GrandTotalAmount_text, TermsOfPayment_Text,Reference_Text,Remark_Text):
    # กำหนดที่อยู่ของไฟล์เทมเพลต Excel
    xlsx_template_path = "Invoice_Form.xlsx"
    # กำหนดที่อยู่ของไฟล์ Excel ที่จะบันทึกข้อมูล
    timestamp = datetime.now().strftime("%d_%m_%Y_%H_%M_%S_")

        
    output_path = fr"\\N2N-BANK\Users\Administrator\Downloads\Excel\Invoice\Invoice_{timestamp}.xlsx"
    path = r"\\N2N-BANK\Users\Administrator\Downloads\Excel\Invoice"
    ...
    sheet.cell(row=12, column=9, value=DocumentNO_text)
    sheet.cell(row=12, column=3, value=name_text)
    sheet.cell(row=13, column=9, value=Date)
    sheet.cell(row=13, column=3, value=Address) 
    sheet.cell(row=15, column=4, value=TaxID)
    sheet.cell(row=15, column=9, value=Reference_Text)

    sheet.cell(row=36, column=12, value=TotalAmount_text)
    sheet.cell(row=40, column=12, value=VATAmount_text)
    sheet.cell(row=42, column=12, value=GrandTotalAmount_text)
    sheet.cell(row=14, column=3, value=tel_text)
    sheet.cell(row=14, column=9, value=TermsOfPayment_Text)

    #Remark
    sheet.cell(row=37, column=3, value=Remark_Text)
    ...
    sheet.cell(row=42, column=5, value=grand_total_text_thai_with_comma  + '  บาท')
    ...
    for i, item_data in enumerate(table_data, start=1):
        # แถวที่จะแทรกข้อมูล
        # กำหนดจำนวนเงินในตาราง
        
        unit_price = float(item_data[4])
        quantity = int(item_data[2])
        amount = float(item_data[3]) * float(item_data[4])
        ...
        sheet.cell(row=current_row, column=description_col_num, value=item_data[5])  # ProductCode

        # กำหนด Description 
        description_col_num = 5  # Column C
        sheet.cell(row=current_row, column=description_col_num, value=item_data[1])  # Description

        # กำหนด Quantity 
        quantity_col_num = 10  # Column H
        sheet.cell(row=current_row, column=quantity_col_num, value=item_data[3])  # Quantity

        Unit_Price = 11  # Column H
        sheet.cell(row=current_row, column=Unit_Price, value=formatted_unit)  # UniPrice

        # กำหนด Amount 
        amount_col_num = 12 # Column J
...
#ต้นฉบับ
def write_to_excel_manuscript(name_text, Date, Address, DocumentNO_text, TaxID, tel_text, TotalAmount_text, VATAmount_text, GrandTotalAmount_text, TermsOfPayment_Text,Reference_Text,Remark_Text):
    # กำหนดที่อยู่ของไฟล์เทมเพลต Excel
    xlsx_template_path = "Invoice_Formmanuscript.xlsx"
    # กำหนดที่อยู่ของไฟล์ Excel ที่จะบันทึกข้อมูล
    timestamp = datetime.now().strftime("%d_%m_%Y_%H_%M_%S_")

        
    output_path = fr"\\N2N-BANK\Users\Administrator\Downloads\Excel\Invoice\Invoice_Formmanuscript{timestamp}.xlsx"
    path = r"\\N2N-BANK\Users\Administrator\Downloads\Excel\Invoice"
    ...
    document_type = "Quotation"
       # เรียกฟังก์ชัน convert_excel_to_pdf เพื่อแปลง Excel เป็น PDF
    pdf_output_path = f"output_{document_type}_form_template.pdf"
    convert_excel_to_pdfmmanuscript(output_path, pdf_output_path)
...
# หาขนาดหน้าจอปัจจุบันของผู้ใช้
screen_width = root.winfo_screenwidth()
screen_height = root.winfo_screenheight()

# กำหนดขนาดหน้าต่างให้เต็มจอ
root.attributes('-fullscreen', True)  # ตั้งค่าให้หน้าต่างเต็มหน้าจอ
...
image_Quotation = PhotoImage(
    file=relative_to_assets("Invoice_Text.png"))
...
searchimage = Button(
    root,
    image=search_image,
    command=Customers.AddCustomer,  # เมื่อคลิกที่ปุ่มนี้จะเรียกใช้ฟังก์ชัน login
...
def update_document_number():
    ...
    DocumentNO.configure(state='normal')  # ตั้ง state เป็น 'normal' เพื่อให้สามารถแก้ไขได้
    ...
    DocumentNO.configure(state='readonly')  # ตั้ง state เป็น 'readonly' เพื่อให้ไม่สามารถแก้ไขได้
    DocumentNO.configure(readonlybackground="white")  # กำหนดสีพื้นหลังเป็นสีขาวเมื่ออยู่ในสถานะ readonly
...
image_Remark_INvoice = PhotoImage(
    file=relative_to_assets("Remark.png"))
imageRemark_INvoice = canvas.create_image(
    400.0,
    710.0,
    image=image_Remark_INvoice
)
# สร้าง Text widget สำหรับ Remark
Remark_INvoice = Text(
    bd=0,
    bg="#FFFFFF",
    fg="#000000",
    highlightthickness=0,
    wrap="word"  # ทำให้คำไม่ถูกตัดกลางคำเมื่อขึ้นบรรทัดใหม่
)
Remark_INvoice.place(
    x=305.0,
    y=680.0,
    width=190.0,
    height=60.0
...
image_Reference = PhotoImage(
    file=relative_to_assets("Ref.png"))
imageReference = canvas.create_image(
    1140.0,
    385.0,
...
button_addnewitem = Button(
    ...
    command=Products.addnewProducts,  # เมื่อคลิกที่ปุ่มนี้จะเรียกใช้ฟังก์ชัน addnewProducts ภายในคลาส Products
    borderwidth=0,  # กำหนดความกว้างของเส้นขอบเป็น 0 เพื่อลบขอบเทาๆ
    highlightthickness=0,  # กำหนดความกว้างของ highlight ให้เป็น 0 เพื่อลบเส้นขอบเทาๆ
)
